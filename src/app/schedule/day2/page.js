...
export default function Day1() {
  useEffect(() => {
    ...
    for (let i = 0; i < vectors.length; i++) {
      ...
      ctx.fillStyle = "#156693";
      ctx.fillRect(
        vectors[i][vectors[i].length - 1]["x"] - 7.5,
        vectors[i][vectors[i].length - 1]["y"] - 7.5,
        15,
        15
      );
      ...
      ctx.fillText(
        events[i],
        vectors[i][vectors[i].length - 1]["x"] + 14,
        vectors[i][vectors[i].length - 1]["y"] + 8
      );
      ctx.font = "20px Inter";
      ctx.fillStyle = "#156693";
      ...
      ctx.fillText(
        `Venue: ${venue[i]}`,
        vectors[i][vectors[i].length - 1]["x"] + 50,
        vectors[i][vectors[i].length - 1]["y"] - 40
      );
      ...
      ctx.fillText(
        `Time: ${Time[i]}`,
        vectors[i][vectors[i].length - 1]["x"] + 50,
        vectors[i][vectors[i].length - 1]["y"] + 33
      );
    }
